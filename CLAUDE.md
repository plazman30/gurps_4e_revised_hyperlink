...
| `convert_pdf_to_grayscale.py` | **Entirely unrelated to hyperlinking.** Converts a PDF to grayscale by shelling out to Ghostscript's `pdfwrite` device (good color conversion, keeps `/Link` annotations intact by default), then uses `pikepdf` to copy the source's Info-dict/XMP metadata and `/PageLabels` back onto the result -- Ghostscript's own full re-interpretation of the file silently drops both, the same category of "clean run, silently wrong/incomplete output" this whole project keeps finding. Requires the actual Ghostscript binary (`gs`) on PATH -- a system package (`brew install ghostscript` on macOS), not something `requirements.txt`/pip can provide, unlike every other dependency in this repo. **Ghostscript's own CLI has a real footgun worth knowing before editing `run_ghostscript()`'s command list: it processes switches strictly left to right, and `-f <input>` triggers immediate interpretation of the input file the instant it's reached -- anything listed after it, including `-sOutputFile=...`, is silently never applied.** The current argument order already puts `-sOutputFile=...` before the trailing `-c ... -f <input>` pair for exactly this reason; preserve that ordering in any future edit. `--keep-producer` is a documented no-op placeholder, not wired to anything yet -- `/Producer` always ends up as Ghostscript's own string regardless of the flag. Interactively prompts before converting (`prompt_yes_no()`/`prompt_page_number()` in `main()`): asks whether the front/back cover should be converted to grayscale too (default yes); answering no asks which page is the front cover (default 1) and which is the back cover (default: the last page, or the literal `none` if there isn't a separate one), each validated against the PDF's real page count. Any page picked this way is swapped back into the final output from the *original* source file, not the Ghostscript-converted one -- `out.pages[idx] = src.pages[idx]` inside `restore_metadata_and_labels()`, a direct pikepdf page-object copy, not a re-render, so that page's original color, text, and links all come through byte-faithful. **First implementation used `out.pages[idx] = out.copy_foreign(src.pages[idx])` instead, confirmed a real bug the moment it was actually run rather than just read**: pikepdf's `copy_foreign()` explicitly refuses a `Page` object (`NotImplementedError: Use pikepdf.Pdf.pages interface to copy pages from one PDF to another`), so every real run asking to keep a cover in color crashed with a traceback right after the Ghostscript step, before any output file was written -- which is why the reported symptom looked like "it converted the covers anyway": whatever file was already sitting at the output path (e.g. from an earlier all-grayscale run) was what got inspected, not a file this run actually produced. `pikepdf.Pdf.pages` already does its own foreign-page import when you assign directly (`out.pages[idx] = src.pages[idx]`, no `copy_foreign()` call needed or supported for this) -- confirmed against a synthetic 3-page red/green/blue test PDF: with front=1/back=3 kept in color, the output's page 1 and page 3 sampled as pure red/blue while the middle page came back gray, and the crash is gone. This assumes Ghostscript never changes the page count during conversion; `restore_metadata_and_labels()` raises immediately if `len(src.pages) != len(out.pages)` rather than silently mapping a cover index to the wrong physical page.

**A user-reported gap -- "the output has JPX images, I want none" -- turned into a real, confirmed bug with a genuinely wrong first fix, not just a missing feature.** Scanned every real PDF in this repo's test corpus for existing JPEG2000 (`/JPXDecode`) images to find a real reproduction case rather than guessing: 3 books have them in their source (`Mongoose Traveller 1E - MGP3800 - Core Rulebook.pdf`, `MongooseTraveller-2E-AdventureClassShips.pdf`, `Traveller_MgP_2300AD_Tools_for_Frontier_Living_MGP20002.pdf`), and running each through the actual grayscale pipeline found that two of the three have Ghostscript convert every JPX image away on its own -- only *Tools for Frontier Living* has 5 that survive completely untouched into the final output. Root cause: those 5 use a single-channel `DeviceN [/Black] -> DeviceGray` separation colorspace (a "how much black ink" tint value, not a direct DeviceGray sample), and Ghostscript's `-sColorConversionStrategy=Gray` decides an image already in a gray-compatible colorspace needs no work at all -- and "no work" turns out to mean the original bytes, JPX encoding included, are passed straight through unmodified, not just that pixel *values* are left alone the way "already correct" suggests.

Fixed with `strip_residual_jpx()`: find every image object still carrying `/JPXDecode` in the Ghostscript output (regardless of *why* Ghostscript left it, so this catches any future combination that triggers the same pass-through decision, not just this one colorspace) and force a real decode + re-encode. **The first version of this fix decoded the raw JPX codestream directly (`fitz.Pixmap(obj.read_raw_bytes())`) and used its samples as the new image data -- confirmed a real, serious bug by rendering the "fixed" output and comparing it to the original side by side (not just checking the filter was gone), and it came out as a full photographic negative.** A `DeviceN [/Black]` separation's raw sample is an *ink amount* (0 = no ink = white, 255 = full ink = black) -- the literal opposite sense of a plain DeviceGray sample (0 = black, 255 = white) -- so copying ink-amount bytes straight into gray-level bytes with no transform in between inverts the whole image. Fixed properly by opening the same file fresh with PyMuPDF and building the pixmap via `fitz.Pixmap(doc, xref)` (MuPDF's own decoder, which preserves the image's real native colorspace -- including its `DeviceN` tint-transform function -- rather than a context-free codestream dump), then converting *that* through `fitz.Pixmap(fitz.csGRAY, pix)`, which is the step that actually evaluates the tint-transform function into a genuine gray value instead of assuming the raw bytes already are one. Re-verified against the real book: rendered all 5 previously-affected pages and confirmed each one now visually matches the original artwork (not just that the pixel byte count looks plausible), re-confirmed zero `/JPXDecode` objects remain anywhere in the file (not just on the 5 known pages), and re-ran the two already-clean JPX-source books plus a JPX-free book afterward to confirm page count, links, and metadata/title are all still exactly preserved -- this only ever touches an image already confirmed broken, nothing else. **General lesson, sharply narrower than it sounds but worth stating precisely: decoding a compressed image's raw bytes in isolation, with no reference to the PDF `/ColorSpace` dictionary that says what those bytes actually mean, is not "decoding the image" -- it's decoding *a* image, and for anything other than a plain Device{Gray,RGB,CMYK} colorspace (Separation/DeviceN tint transforms chief among them, per this project's own established caution elsewhere about evaluating those correctly) that can silently produce a completely different, wrong result while looking successful right up until someone actually looks at the picture.** Usage: `python3 convert_pdf_to_grayscale.py input.pdf [output.pdf] [--keep-producer]` -- output defaults to `<input>-grayscale.pdf` alongside the input when omitted. |
| `pdfsqueeze.py` | **Entirely unrelated to hyperlinking.** A "PDF Squeezer"-style lossy compressor for Linux/macOS, chaining two external tools per file: `qpdf` first (lossless structural cleanup -- object streams, stream recompression, image optimization), then Ghostscript's `pdfwrite` device (the lossy pass -- image downsampling and JPEG re-encoding), parameterized by a `light`/`medium`/`heavy` profile dict (`PROFILES`) approximating PDF Squeezer's own presets by feel, not by measuring against real PDF Squeezer output -- no `.pdfscp` profile file or documented CLI syntax for that tool was ever found to calibrate against. Requires both `qpdf` and Ghostscript (`gs`) as system packages, neither installable via this repo's `requirements.txt`.

    **First real-file test run (this book, `hyperlink_pdf_universal.py`'s own `hyperlink_pdf_mongoose.py` test corpus) surfaced three confirmed bugs the original synthetic-only testing never hit** -- consistent with this whole project's repeated lesson that synthetic tests validate *shape*, not real-world edge cases:
    - **The pipeline aborted outright on a completely ordinary real book.** `run()` treated any nonzero exit code from either external tool as a hard failure, but `qpdf` exits **3** (not 0) whenever it fixes up a merely-imperfect input and still produces valid output -- confirmed on `Traveller Companion Update 2024...pdf`: qpdf printed its own `"operation succeeded with warnings; resulting file may have some problems"` while the resulting file had the *identical* page count, link count (251), and TOC entry count (38) as the source. qpdf's own `--help=exit-status` explicitly documents 2 as "errors detected" vs. 3 as "warnings detected" -- two different things this code conflated. Fixed by adding a `tolerate_codes` parameter to `run()`, passed as `{3}` from `qpdf_cleanup()` only (Ghostscript's call site is untouched -- its own exit codes weren't the problem here), while still surfacing the warning text to stderr rather than swallowing it.
    - **`--verify-links` produced a massive false-positive rate on a real book** -- 238 of 251 links (95%) wrongly reported "missing" on the very first real file tried, even though the true link count was unchanged (251 before, 251 after). Root cause: this book's internal cross-references use `LINK_NAMED` annotations (kind 4, PyMuPDF's encoding for an InDesign-exported named-destination link -- confirmed via the literal `".indd:"` marker inside its own `nameddest` string), and Ghostscript's `pdfwrite` device harmlessly re-encodes every one of these into a plain `LINK_GOTO` (kind 1) during recompression -- same rect, same resolved target page, functionally identical for a reader, just a different annotation encoding. `_link_signature()` included the raw `kind` value in its identity tuple, so a kind-4-to-kind-1 conversion looked like every link on the page was replaced by a completely different, unrelated one. Fixed by treating kinds 1 and 4 as the same category (`"goto"`), keyed on the resolved `page` field both already carry, rather than on which annotation encoding produced it. This also mooted a second, smaller latent bug in the same branch: the old kind-4 case read `link.get("name")`, but PyMuPDF actually stores that field as `"nameddest"` -- so the destination-target part of the fingerprint was silently `None` for every named-destination link even before the kind-mismatch issue existed.
    - **`--subfolders` combined with `--output` could silently clobber files.** `resolve_output()`'s directory-output branch wrote every file to `out_path / src.name` -- just the bare filename, dropping which subfolder it came from. Confirmed: two different subfolders each containing an `issue1.pdf` (a realistic case for a per-series comics/sourcebook folder layout, exactly what `--subfolders` exists to support) both resolved to the identical output path, and the second one processed silently overwrote the first with no warning at all -- real, silent data loss, not a cosmetic issue. Fixed the same way `hyperlink_pdf_universal.py --batch --recursive` already handles this in this same repo: `collect_inputs()` now returns `(path, rel_name)` pairs, where `rel_name` is the path relative to whichever directory argument it was found under, and `resolve_output()` joins that relative path onto the output directory (creating any needed subfolder) instead of just the bare filename.

    All three were found and fixed by actually running the tool against a real book with a real link-dense cross-reference structure -- exactly the single highest-value untested case the original draft notes for this script already flagged as outstanding (only synthetic 2-link test PDFs had been tried before). Re-verified afterward: the same real book now completes successfully end-to-end with `--profile medium --verify-links` (qpdf's warning is printed but no longer fatal, and the tool reports "links/TOC/pages verified intact" -- confirmed independently via a direct pymupdf page/link/TOC count comparison: 186/186 pages, 251/251 links, 38/38 TOC entries), a synthetic red/green/blue-page `--subfolders` reproduction (two identically-named files in different subfolders) now lands at two distinct paths with distinct content instead of one clobbering the other, and the full prior regression battery (single-file default naming, `--replace` + `.bak` backup, flat multi-file `--output`, non-recursive folder input) was re-run afterward with no change in behavior. **General lesson, consistent with this whole project's own established pattern (see "No test PDF in this repo" below): a synthetic test with a handful of links validates the mechanism's *shape*, but a real book's actual annotation conventions (named destinations, qpdf warnings on real-world structural quirks, reused filenames across a real folder hierarchy) are what actually break code that "looks correct on paper" -- the single highest-value next test for any new PDF-processing script in this repo is a real file with real structural quirks, not a bigger synthetic one.**

    **A follow-up question -- "does this retain metadata and page labels?" -- turned up a fourth real gap, closed the same way: by actually testing, not reading the code and assuming.** The pipeline had no metadata/`/PageLabels` handling at all (no `pikepdf` import, unlike `convert_pdf_to_grayscale.py`'s own function of the same name). Direct testing (a synthetic PDF with real roman/arabic `/PageLabels`, plus the same real book above) found: Ghostscript preserves `/PageLabels` and Info-dict Title/Author/Subject/Keywords on their own in the simple case, **but regenerates XMP metadata from scratch rather than passing the original through** -- confirmed on the real book, which has no `dc:title` anywhere in its actual source (neither XMP nor Info dict), yet Ghostscript's output fabricated one: `dc:title: 'Untitled'`. That's invented metadata, not just dropped metadata -- worth taking as seriously as a lost link. Fixed by adding `restore_metadata_and_labels()`, called from `compress_one()` as a third stage after `qpdf`+Ghostscript (writing to a `stage2.pdf` intermediate first, then this function produces the real final output) -- the same `pikepdf`-based pattern as `convert_pdf_to_grayscale.py`'s function of the same name: `open_metadata()` on both files, restore from the original's docinfo + real XMP, and copy `/PageLabels` across if present.

    Getting this right took three rounds of testing catching three more real bugs, not one clean pass:
    - **`copy_foreign()` on `/PageLabels` raised `ForeignObjectError: ... called with direct object handle`** on the synthetic test file specifically -- `pikepdf.new()` plus a plain dict assignment (`pdf.Root.PageLabels = pikepdf.Dictionary(...)`) creates it as a *direct* object rather than an indirect one, and `copy_foreign()` requires indirect. (A real book's own `/PageLabels`, built by production tools that emit most substantial dicts as indirect objects, may not hit this -- but nothing guarantees it never will, and the fix is cheap.) Fixed with an `is_indirect` check, calling `src.make_indirect(labels)` first when needed -- safe since `src` is only ever read from, never saved.
    - **The restore looked complete on inspection mid-function but wasn't once actually saved and reopened**: `out_meta.load_from_docinfo(src.docinfo)` followed by `out_meta.update(src_meta)` correctly set the in-memory XMP object's `pdf:Producer` to the *original* value (confirmed directly, mid-call) -- but pikepdf's own `open_metadata(update_docinfo=True)` sync-on-close unconditionally re-stamps `/Producer`, `/CreationDate`, and `/ModDate` with pikepdf's own current identity the moment the `with` block exits, regardless of what was just set. A later direct `docinfo["/Producer"] = ...` write after the block doesn't reliably survive `save()` either. This is a pikepdf-level behavior, independent of and in addition to Ghostscript's own identical habit on these three fields -- so the net result matches, rather than diverges from, `convert_pdf_to_grayscale.py`'s own already-documented limitation there (its `--keep-producer` flag being a no-op turns out to be the *same* underlying cause, not a separate, fixable gap this script avoided).
    - **`load_from_docinfo()`/`update()` are both purely additive -- neither ever deletes a key the source doesn't mention -- so Ghostscript's fabricated `dc:title: 'Untitled'` survived the restore untouched**, merely sitting underneath whatever real data got added on top of it (and on the real book specifically, nothing *did* get added on top, since the real source has no title at all -- so the fabricated one came through completely unfiltered, into both XMP and, worse, freshly synced into the Info dict's `/Title` too). Fixed with `out_meta.clear()` immediately before `load_from_docinfo()`/`update()`, wiping Ghostscript's invented fields first rather than merging on top of them.

    All three were caught by re-running the exact same real book end-to-end after each fix, not by re-reading the code and trusting it looked right -- confirmed final state: `dc:title` correctly `None`/`None` (matching the real source) instead of a fabricated `'Untitled'`, `xmp:CreatorTool` correctly carried through as the genuine original value, `/PageLabels` round-trips correctly on the synthetic test, and the full existing regression battery (real-book links/TOC/page-count, `--subfolders` collision fix, `--replace`+backup, multi-file `--output`) was re-run afterward with no change. **A fourth apparent discrepancy during this testing turned out to be a bug in the test methodology, not in `pdfsqueeze.py` -- worth recording as its own lesson**: chaining `pikepdf.open(path).docinfo.get(...)` in one expression let Python garbage-collect the temporary `Pdf` object before `.get()` ran, silently returning empty/default data instead of raising (occasionally surfacing as an explicit `"operation ... attempted on object of type destroyed: treating as empty"` error instead, when GC timing was different) -- `qpdf --check`, `qpdf --show-object`, and `pypdf` all confirmed the actual file bytes were completely correct throughout. **The general rule this establishes for any future `pikepdf` scratch-testing in this repo: always bind `pikepdf.open(...)` to a variable and keep it referenced for as long as anything derived from it (a `Dictionary`, `.docinfo`, etc.) is still being used -- never chain the open and a later attribute access across an expression boundary where the `Pdf` object itself has no surviving reference.**

    Known unresolved gaps carried over from the original draft, not yet independently re-verified: profile numbers in `PROFILES` are still an unvalidated approximation of PDF Squeezer's actual presets (no reference `.pdfscp` file has been obtained to calibrate against); no handling for password-protected/encrypted PDFs; `--verify-links`'s fingerprint still doesn't account for a link's target page shifting due to a page being merged/removed, though that's out of scope for a pure compression pipeline that never changes page count. Usage: `python3 pdfsqueeze.py FILE [FILE ...] --profile {light,medium,heavy} [--output DIR] [--replace] [--no-backup] [--subfolders] [--verify-links] [--verbose]`. |
