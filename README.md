...
|---|---|
| `hyperlink_pdf_mongoose.py` | The version to use for **Mongoose Publishing's Traveller line — both 2nd edition and 1st edition** (2e: Traveller Core Rulebook, Traveller Companion, High Guard, Aliens of Charted Space, the 2300AD boxed set; 1e: Core Rulebook, Alien Modules, the numbered "Little Black Book" reprints, Sector sourcebooks, and similar) instead of `hyperlink_pdf_universal.py`. Same overall approach and command-line usage (single file or `--batch`, same CSV report), but adapted to that line's actual conventions: recognizes spelled-out `page NNN`/`pages NNN-NNN` wording (not just `p.`/`pp.` — though 1e books that use the abbreviated style still work too), understands a "pages 152-3" abbreviated-range style, finds the Table of Contents/Index by heading size and position (or, for a smaller faux-bold heading style some books use instead of a bigger font, by detecting the heading word rendered twice at identical coordinates) instead of a repeated footer word, and catches four distinct cross-book citation shapes — the other book's title *after* the page number ("page 149 of the Traveller Core Rulebook"), a bare title with no connector at all before the page number ("Characters & Equipment page 38"), a same-boxed-set shorthand for citing a sibling PDF ("Book2 page 23"), and 1st-edition's `"Book N: <Title>"` metadata naming convention. Correctly tells a book's own chapter name apart from a citation of a different product even when both look identical in shape (e.g. "page 32 of the Vargr Race chapter" inside the book that *has* a Vargr Race chapter). Not a generic "any publisher" script — it's specific to this line, the same way `hyperlink_pdf_universal.py` is specific to GURPS. |
...
| `fix_page_labels.py` | Standalone utility, unrelated to hyperlinking: interactively rebuilds a PDF's `/PageLabels` — the page-number display a PDF viewer (Acrobat, Preview, etc.) shows in its navigation bar and thumbnail panel — for the common cover + roman-numeral front matter + arabic body layout. Asks whether page 1 is the front cover, where the back cover is (last page, a specific page, or none at all — it doesn't have to be at the end), and which PDF page is actually printed page "1" (pre-filled with a guess from scanning each page's footer for a printed number, when PyMuPDF is available — press Enter to accept it or type over it), then labels both covers as the constant text "Cover", numbers everything between the front cover and page 1 with sequential lowercase roman numerals, and numbers everything from page 1 onward with sequential arabic numbers — skipping over the back cover wherever it falls (front matter, body, or the end) without leaving a gap in the surrounding numbering. Add `--tui` for a full-screen [Textual](https://textual.textualize.io/) form instead of the default plain y/n prompts — the same three questions as one editable screen (a checkbox, a 3-way choice for the back cover, a page-1 field), mouse or keyboard, ending on a preview screen showing every resulting page-label range before anything is written; `Esc` quits from either screen, and rejecting the preview goes back to the form with everything you entered still there. Needs a real terminal; `textual` is already in `requirements.txt` and works cross-platform with no extra Windows-specific package. **Note:** the hyperlinker scripts don't read `/PageLabels` at all — they detect page numbering from the actual printed footer/header text on each page — so this doesn't affect hyperlinking one way or the other; it's purely about what a PDF viewer's own UI displays. |
...
| `combine_gurps_basic_set.py` | Merges the GURPS Basic Set, 4th Edition PDFs (Characters + Campaigns) into one combined book: drops a handful of cut-content pages, splices each book's front matter and table of contents together, moves both copyright/title (`II`) pages to the very end, relabels the merged front matter with sequential lowercase roman numerals, and sets the file's title/author/subject metadata plus an open-to-Contents action — all while keeping every other page's original page label intact. Book-specific (hardcodes which page labels move/delete for this exact pair of books), not a general-purpose PDF merger. Add `--hyperlink` to also link every in-text page/chapter reference in the merged file afterward (off by default — without it, the output is just the combined PDF with no links). This isn't just `hyperlink_pdf_universal.py` called on the result: that script's page-numbering detection assumes one constant offset for the whole book, which doesn't hold once two separately-paginated books are spliced together, so it has its own self-contained copy of the linking logic that looks up each page number directly instead. See `python3 combine_gurps_basic_set.py --help` for full usage. |
...
| `castles_and_crusades_background_layer.py` | Standalone utility, unrelated to hyperlinking: tags (or permanently deletes) the full-page decorative background — parchment texture, paper grain, etc. — on each page of a print-production PDF, so it can be toggled off in a layers-aware viewer or stripped out entirely to shrink the file. Built and verified against Troll Lord Games' Castles & Crusades line (Castle Keeper's Guide, Adventurers Backpack, Monsters & Treasure), but the detection is geometry-based (tracks the actual page transform to compute what each drawn object really covers, rather than guessing from draw order or object naming) so it should generalize to other publishers' similar print-to-PDF output. Two modes: default tags the background as a toggleable Optional Content Group layer (reversible; requires a viewer that supports PDF layers — **Preview.app on macOS does not**, use Acrobat/PDF Expert/Xodo/PDF-XChange instead, or use `--remove` to see the effect in any viewer); `--remove` deletes it outright (irreversible, smaller file). Usage: `python3 castles_and_crusades_background_layer.py input.pdf output.pdf [--remove]`. |
| `extract_gurps_wraparound_cover.py` | Standalone utility, unrelated to hyperlinking: recovers hidden wraparound cover art (back cover + spine + front cover, all on one page) from GURPS/Steve Jackson Games PDFs whose `/CropBox` is deliberately set smaller than the `/MediaBox` so ordinary viewers only ever show the front-cover slice — some self-hosted book-server thumbnailers (BookOrbit, Grimmory, Kavita) render straight from the MediaBox instead, which is how the full art becomes visible there but nowhere else. Auto-detects the wraparound page by comparing every page's MediaBox area to its CropBox area (a page whose MediaBox is notably larger is almost certainly hiding extra art outside the crop), extracts just that page into its own PDF with the CropBox expanded to match the MediaBox — copying the original vector objects, images, and fonts through untouched, nothing re-rendered — then also renders a companion SVG with every text run outlined to vector paths, so the SVG has zero font dependencies and is safe to open or share without the original fonts installed. Some printings split the wraparound spread across **two** pages instead of one (confirmed on a real book) — each page draws only its own half with the rest of its MediaBox left blank; the script detects this (most of the "revealed" area coming out as one solid color) and automatically combines the two pages into one complete, seamless cover — still a vector-level copy, not a rasterized blend, so text stays live text. If it can't find the other half, it says so instead of silently handing back a half-blank cover. Use `--list` to inspect every page's MediaBox/CropBox ratio before extracting, or `-p PAGE_NUM` to pick a page explicitly if auto-detection guesses wrong. Usage: `python3 extract_gurps_wraparound_cover.py MyBook.pdf [-o OUTPUT_PREFIX] [-p PAGE_NUM] [--list] [--min-ratio N]`. |
| `gurps_4e_revised_tag_borders_ocg.py` | Standalone utility, unrelated to hyperlinking, built specifically against `GURPS_Basic_Set_Fourth_Edition_Revised.pdf`'s page design: wraps that book's colored decorative border (the maroon/gold margin gradient, and the picture-frame outlines around callout/sidebar boxes) in toggleable PDF layers instead of deleting anything, so any viewer with a Layers panel (Acrobat, PDF-XChange) can switch them off. Also produces a third, off-by-default layer working around a real quirk in the source file: footer/header text has no color of its own — it just inherits whatever fill was last set (which happens to be the page's white background), so it silently goes invisible if the border layer is turned off. Turning on "Border Text (fallback)" draws a black copy of that same text on top, restoring readability without the border. A fourth, also off-by-default layer, "Solid Edge Border," adds a flat 10pt solid-color strip flush against the page's *outside* (fore-)edge only — never the gutter side, and never top/bottom — alternating which physical side that is by recto/verso, in whatever color that page's own border gradient resolves to. An independent addition, not a substitute for the other layers. Never rasterizes anything — edits content streams directly via `pikepdf`. Add `--tui` for a plain [Textual](https://textual.textualize.io/) progress bar instead of one printed line per page — useful for a full ~600-page book, where plain mode's scrolling output makes it hard to tell how far along a long run actually is; once it reaches 100% it closes itself automatically after a few seconds (or press `q` to close it immediately), and the usual per-page/summary output still prints normally afterward. Usage: `python3 gurps_4e_revised_tag_borders_ocg.py input.pdf output.pdf [--pages START-END] [--dry-run] [--tui]` (page range is 0-indexed, inclusive). |
| `gurps_4e_revised_flatten_gradients.py` | Standalone utility, unrelated to hyperlinking, built for the same file as `gurps_4e_revised_tag_borders_ocg.py` above: permanently replaces every gradient fill with a plain solid color (unlike `gurps_4e_revised_tag_borders_ocg.py`, this rewrites the file rather than making the border toggleable) — useful for print-shop workflows or viewers that render gradients poorly. Picks the color from the strongest end of each gradient's ramp and resolves it down through whatever colorspace the PDF defines (including spot-color/DeviceN tint transforms) to a plain CMYK/RGB/Gray fill. Usage: `python3 gurps_4e_revised_flatten_gradients.py input.pdf output.pdf [--pages START-END] [--dry-run]`. |
| `convert_pdf_to_grayscale.py` | Standalone utility, unrelated to hyperlinking: converts a PDF to grayscale using Ghostscript's `pdfwrite` device, then uses `pikepdf` to copy the original document metadata (Info dict + XMP) and `/PageLabels` back in — Ghostscript's own conversion drops both silently, since it fully re-interprets the file. Prompts interactively before converting: asks whether the front and back cover pages should be converted to grayscale too (default yes); answer no and it asks which page is the front cover (default: page 1) and which is the back cover (default: the last page — type `none` if there isn't one), then leaves exactly those pages in their original, untouched color while everything else becomes grayscale. Requires the Ghostscript binary (`gs`) installed as a system package, separate from this repo's Python dependencies — e.g. `brew install ghostscript` on macOS. Usage: `python3 convert_pdf_to_grayscale.py input.pdf [output.pdf]` — output defaults to `<input>-grayscale.pdf` alongside the input if omitted. |
| `pdfsqueeze.py` | Standalone utility, unrelated to hyperlinking: a "PDF Squeezer"-style lossy compressor, chaining `qpdf` (lossless structural cleanup), then Ghostscript's `pdfwrite` device (image downsampling + JPEG recompression), then a `pikepdf`-based metadata/`/PageLabels` restore pass, per file, at a `light`/`medium`/`heavy` profile. That last pass matters: Ghostscript regenerates XMP metadata from scratch rather than passing the original through — confirmed to fabricate a placeholder title where none existed in the source — so this restores the original Info-dict + XMP metadata and `/PageLabels` afterward (same approach as `convert_pdf_to_grayscale.py`); `/Producer`/`/CreationDate`/`/ModDate` are the one exception that can't be restored this way (a `pikepdf`-level limitation, not something this tool controls). Requires both `qpdf` and Ghostscript (`gs`) as system packages, separate from this repo's Python dependencies. Add `--verify-links` (needs `pymupdf`, already a project dependency) to confirm page count, every link, and TOC/bookmarks survived compression before you trust the result — correctly tolerates Ghostscript's own harmless re-encoding of an internal named-destination link into a plain page-goto link (same target, different annotation form), rather than flagging it as lost. `--replace` overwrites the original in place (keeping a `.bak` copy unless `--no-backup`); `--subfolders` recurses through a folder of PDFs and preserves that subfolder structure under `--output` (so two same-named files in different subfolders don't collide). Usage: `python3 pdfsqueeze.py FILE [FILE ...] --profile {light,medium,heavy} [--output DIR] [--replace] [--subfolders] [--verify-links]`. |
