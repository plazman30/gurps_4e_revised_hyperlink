...
| `tag_borders_ocg.py` | Standalone utility, unrelated to hyperlinking, built specifically against `GURPS_Basic_Set_Fourth_Edition_Revised.pdf`'s page design: wraps that book's colored decorative border (the maroon/gold margin gradient, and the picture-frame outlines around callout/sidebar boxes) in toggleable PDF layers instead of deleting anything, so any viewer with a Layers panel (Acrobat, PDF-XChange) can switch them off. Also produces a third, off-by-default layer working around a real quirk in the source file: footer/header text has no color of its own — it just inherits whatever fill was last set (which happens to be the page's white background), so it silently goes invisible if the border layer is turned off. Turning on "Border Text (fallback)" draws a black copy of that same text on top, restoring readability without the border. Never rasterizes anything — edits content streams directly via `pikepdf`. Usage: `python3 tag_borders_ocg.py input.pdf output.pdf [--pages START-END] [--dry-run]` (page range is 0-indexed, inclusive). |
| `flatten_gradients.py` | Standalone utility, unrelated to hyperlinking, built for the same file as `tag_borders_ocg.py` above: permanently replaces every gradient fill with a plain solid color (unlike `tag_borders_ocg.py`, this rewrites the file rather than making the border toggleable) — useful for print-shop workflows or viewers that render gradients poorly. Picks the color from the strongest end of each gradient's ramp and resolves it down through whatever colorspace the PDF defines (including spot-color/DeviceN tint transforms) to a plain CMYK/RGB/Gray fill. Usage: `python3 flatten_gradients.py input.pdf output.pdf [--pages START-END] [--dry-run]`. |
